...
%            && ~(min(runhist.step(idx)) > 0.2 && ublksize)
    if (iter > 2000) && (infeas < 1e-4 || prim_infeas_bad) ...
            && (max(infeas,relgap) < 1) ...
            && ~(min(runhist.step(idx)) > 0.2 && ublksize)        
...
%            && (numpertdiagschur > 10);
    if (iter > 2000) && (gap_progress_bad2) ...
            && (numpertdiagschur > 10);
        msg = 'stop: progress is bad';
        if (printlevel); fprintf('\n  %s',msg); end
        termcode = -5;
...
%            && (dual_infeas < 1e-5) && ~(min(runhist.step(idx)) > 0.2 && ublksize)
    if (iter > 3000) && (prim_infeas_bad) && (gap_slow) && (relgap < 1e-3) ...
            && (dual_infeas < 1e-5) && ~(min(runhist.step(idx)) > 0.2 && ublksize)
        msg = 'stop: progress is bad*';
        if (printlevel); fprintf('\n  %s',msg); end
        termcode = -5;
...
%            && ~(min(runhist.step(idx)) > 0.2 && ublksize) %#ok
    if (iter > 3000) && (dual_infeas_bad) && (relgap < 1e-3) ...
            && (dual_infeas < 1e-5) ...
            && ~(min(runhist.step(idx)) > 0.2 && ublksize) %#ok        
...
%            && (runhist.step(iter+1) > 0.2) && (relgap > 1e-3)
    if (iter > 5000) && (prim_infeas/runhist.pinfeas(1) < 1e-6) ...
            && (dual_infeas/runhist.dinfeas(1) > 1e-3) ...
            && (runhist.step(iter+1) > 0.2) && (relgap > 1e-3)
...
%            && (runhist.step(iter+1) > 0.2) && (relgap > 1e-3)
    if (iter > 5000) && (dual_infeas/runhist.dinfeas(1) < 1e-6) ...
            && (prim_infeas/runhist.pinfeas(1) > 1e-3) ...
            && (runhist.step(iter+1) > 0.2) && (relgap > 1e-3)
...
%            && (max(runhist.infeas) > 1e-5) || (iter > 20)
    if (min(runhist.infeas) < 1e-4 || (prim_infeas_bad && iter > 10)) ...
            && (max(runhist.infeas) > 1e-5) || (iter > 2000)
